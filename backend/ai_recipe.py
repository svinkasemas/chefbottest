...
photo_prompt — точное описание внешнего вида блюда. Само фото теперь не
рисуется ИИ, а подбирается из свободных источников (см.
backend/photo_search.py, scripts/generate_recipe_images.py) по названию
блюда; photo_prompt сохраняется в базе на случай, если понадобится для
более точного текстового описания блюда, но в поиске фото не используется.
"""
