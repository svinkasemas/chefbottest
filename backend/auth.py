...
def _validate_init_data(init_data: str) -> dict:
    ...
    if not hmac.compare_digest(computed_hash, received_hash):
        logger.warning("Несовпадение подписи. computed=%s received=%s data_check_string=%r", computed_hash, received_hash, data_check_string)
...
async def get_current_user(x_telegram_init_data: str | None = Header(default=None)) -> TelegramUser:
    ...
    if not x_telegram_init_data:
        logger.warning("Запрос без заголовка X-Telegram-Init-Data вообще")
    ...
    try:
        ...
        logger.warning("Ошибка валидации initData: %s | raw=%r", e, x_telegram_init_data)
